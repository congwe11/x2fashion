...
class SpaceTimeUnet(nn.Module):
    ...
    def forward(
        ...
        for init_block, blocks, maybe_attention, downsample, cross_attn in self.downs:
            # x += down_block_additional_residuals[i][:, 0:x.shape[1], :, :, :]
            # i += 1
        ...
        if down_block_additional_residuals is not None:
            ...
            for down_block_res_sample, down_block_additional_residual in zip(
                hiddens, down_block_additional_residuals
            ):  
                # 复制维度
                # down_block_additional_residual = down_block_additional_residual.unsqueeze(2)
                # down_block_additional_residual.expand(-1, -1, down_block_res_sample.shape[2], -1, -1)

                down_block_res_sample += down_block_additional_residual
                new_down_block_res_samples.append(down_block_res_sample)

            hiddens = new_down_block_res_samples
        ...
        if mid_block_additional_residual is not None:
            ...
            # mid_block_additional_residual.expand((-1, -1, x.shape[2], -1, -1))

            mid_block_additional_residual = mid_block_additional_residual[:, :, 0:x.shape[2], :, :]
            x += mid_block_additional_residual
